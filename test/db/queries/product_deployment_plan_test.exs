defmodule DB.Queries.ProductDeploymentPlan.Test do
  ...
  test "get_deployment_plans_for_product- no plans", context do
  ...
  test "get_deployment_plans_for_product- one plan", context do
    product = Repo.insert(%Product{name: "#{UUID.uuid1()}"})
  ...
  test "get_deployment_plans_for_product- multiple plans", context do
    product = Repo.insert(%Product{name: "#{UUID.uuid1()}"})
    plan = Repo.insert(%ProductDeploymentPlan{product_id: product.id, name: "#{UUID.uuid1()}"})
    plan2 = Repo.insert(%ProductDeploymentPlan{product_id: product.id, name: "#{UUID.uuid1()}"})
  ...
  test "get_deployment_plan_by_name- one plan", context do
    product = Repo.insert(%Product{name: "#{UUID.uuid1()}"})
    plan = Repo.insert(%ProductDeploymentPlan{product_id: product.id, name: "#{UUID.uuid1()}"})
